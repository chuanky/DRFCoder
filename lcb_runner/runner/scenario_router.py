...
def build_prompt_benchmark(
    ...
    list[CodeExecutionProblem] | list[CodeGenerationProblem] | list[TestOutputPredictionProblem],
    callable,
]:
    ...
    if scenario == Scenario.codegeneration:
        ...
        if not_fast:
            ...
            benchmark = load_code_generation_dataset(args.release_version, start_date=args.start_date, end_date=args.end_date)
    ...
    if scenario == Scenario.codegeneration:
        ...
        combined_results = [
            (
                ...
                [
                    (extract_code(output[0], model.model_style) if type(output) is list else extract_code(output, model.model_style))
        ...
        combined_results = [
            (
                outputs_list,
                [extract_execution_code(output, model.model_style, cot=cot_code_execution) for output in outputs_list],
            )
            for outputs_list in results
        ]
    else:
...
def get_codegen_metrics(args, benchmark: list[CodeGenerationProblem], pred_data: list[dict]):
    benchmark_map = {item.question_id: item for item in benchmark}
    question_ids = [item["question_id"] for item in pred_data]
    generations = [item["code_list"] for item in pred_data]
    eval_samples = []
    for quesiton_id in question_ids:
        eval_samples.append(benchmark_map[quesiton_id].get_evaluation_sample())
